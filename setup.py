...
if sys.version_info[0] == 2 and sys.version_info[1] < 7:
    raise Warning(
        'This software is not tested on your version of Python.  Be careful!')
...
package_init_file_name = reduce_fun(os.path.join,
                                    [exec_startdir, 'Lib', 'ldap',
                                     '__init__.py'])
...
class OpenLDAP2:
    library_dirs = [r'libraries\libldap', r'libraries\libldap_r', ]
    include_dirs = ['include', r'include\ac']
    extra_compile_args = []
    extra_link_args = []
    extra_objects = []
    libs = ['ldap', 'lber']
...
setup(
  ...
  pyldap is a fork of python-ldap, and provides an object-oriented API to 
  access LDAP
  ...
  """,
    ...
    classifiers=[
        ...
        # Note: when updating Python versions, also change .travis.yml and
        # tox.ini
    ...
    ext_modules=[
        Extension(
            ...
            runtime_library_dirs=(not sys.platform.startswith(
                "win")) * LDAP_CLASS.library_dirs,
            define_macros=(
                LDAP_CLASS.defines +
                (
                ...
                [('HAVE_LIBLDAP_R', None)] +
                (
                    'sasl' in LDAP_CLASS.libs or 'sasl2' in
                    LDAP_CLASS.libs or 'libsasl' in LDAP_CLASS.libs
                ) *
                [('HAVE_SASL', None)] +
                (
